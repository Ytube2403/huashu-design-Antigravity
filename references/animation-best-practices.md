...
> 基于 Anthropic 官方三支产品动画（Antigravity Design / Antigravity Desktop / Antigravity for Word）
...
- Antigravity Design 展示 tweak 参数、拖滑块（不是一键生成完美结果）
- Antigravity 展示代码报错 + AI 修复（不是一次成功）
- Antigravity for Word 展示 Redline 红删绿增的修改过程（不是直接给最终稿）
...
### 配方 A · Apple Keynote 戏剧式（Antigravity Design 类）
...
### 配方 C · 办公效率叙事式（Antigravity for Word 类）
